...
int CEpollServer::StartServer(const char* pszAddr, uint16_t unPort)
{
    ...
    EV_SET(&ev[0], fd, EVFILT_READ, EV_ADD|EV_ENABLE, 0, 0, (void*)(intptr_t)fd);
    EV_SET(&ev[1], fd, EVFILT_WRITE, EV_ADD|EV_ENABLE, 0, 0, (void*)(intptr_t)fd);

    if(kevent(m_epfd, ev, 2, NULL, 0, NULL) == -1)
    {
...
int CEpollServer::Service(const char* pszAddr, unsigned int unPort)
{
...
#ifdef __APPLE__
    struct kevent ev;
    struct kevent events[MAX_EPOLL_SIZE];

    struct timespec timeout;
    timeout.tv_sec = 50 / 1000;
    timeout.tv_nsec = (50 % 1000) * 1000 * 1000;
#else
    struct epoll_event ev;
    ...
    // 记录每一帧消耗的时间
    int nTIME_PER_FRAME = 1000000 / 60;
    CGetTimeOfDay time_frame;
    ...
    while (!m_bShutdown)
    {
...
#ifdef __APPLE__
        int nfds = kevent(m_epfd, NULL, 0, events, event_count, &timeout);
#else
        ...
        for (int n = 0; n < nfds; ++n)
        {
...
#ifdef __APPLE__
            int fd = (int)(intptr_t)events[n].udata;
#else
            ...
            switch(tfd)
            {
...
#ifdef __APPLE__
                    if(this->HandleFdEvent(fd, events[n].filter, mb) != 0)
#else
                ...
                {
#ifdef __APPLE__
                    if(this->HandleMailboxEvent(fd, events[n].filter, mb) != 0)
#else
        ...
        int frameTime = time_frame.GetLapsedTime();
        if (frameTime < nTIME_PER_FRAME)
        {
            usleep(nTIME_PER_FRAME - frameTime);
        }
...
int CEpollServer::HandleNewConnection(int fd)
{
    ...
    if(kevent(m_epfd, &ev, 1, NULL, 0, NULL) == -1)
    {
...
int CEpollServer::HandleFdEvent(int fd, uint32_t event, CMailBox* mb)
{
    ...
    if(ret < 0)
    {
        ...
        EV_SET(&ev[1], fd, EVFILT_WRITE, EV_DELETE, 0, 0, (void*)(intptr_t)fd);

        if(kevent(m_epfd, ev, 2, NULL, 0, NULL) == -1)
        {
            ERROR_PRINT2("Failed to delete fd from kqueue");
        }
#else
        epoll_ctl(m_epfd, EPOLL_CTL_DEL, fd, NULL);
#endif
