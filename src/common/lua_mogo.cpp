...
void ParseNode(lua_State* L, XMLNode* p)
{
     if(p->ToElement())
     {
         //cout << p->Value() << "-----" << endl;

         XMLNode* p2 = p->FirstChild();
         if(p2 && p2->ToText() )
         {
             //cout << p2->Value() << endl;
             //key : value
...
void ParseNodeByKey(lua_State* L, XMLNode* p, const char* pszKey)
{
    if(p->ToElement())
    {
        //cout << p->Value() << "-----" << endl;
        ...
        XMLNode* pKeyNode = p->FirstChildElement(pszKey);
        if(pKeyNode && pKeyNode->ToElement())
        {
            XMLNode* pKeyValueNode = pKeyNode->FirstChild();
            if(pKeyValueNode && pKeyValueNode->ToText() )
            {
                //value还有嵌套结构,生成子table
                lua_newtable(L);
...
bool ReadXmlKeyValue(XMLNode* p, lua_State* L, bool bAtoi)
{
    if(p != NULL && p->ToElement())
    {
        XMLNode* p2 = p->FirstChild();
        //LogDebug("ReadXmlKeyValue", "key=%s;value=%d", p->Value(), atoi(p2->Value()));
        if(p2 != NULL && p2->ToText())
        {
            lua_pushstring(L, p->Value());
...
const char* _ReadXmlValue(XMLNode* p)
{
    if(p != NULL && p->ToElement())
    {
        XMLNode* p2 = p->FirstChild();
        ...
        if(root)
        {
            ...
            for(; node != NULL; node = node->NextSibling())
            {
                ...
                if( strcmp(pszNodeName, "entities") == 0)
                {
                    ...
                    for(; p2 != NULL; p2 = p2->NextSibling())
                    {
                        XMLNode* p3 = p2->FirstChildElement("id");
                        if(p3)
                        {
                            lua_newtable(L);
