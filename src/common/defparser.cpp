...
namespace mogo
{
    ...
    void CDefParser::ParseEntitiesXml(const char* pszFileName)
    {
        ...
        XMLElement* ety = root->FirstChildElement();
    ...
    SEntityDef* CDefParser::ParseDef(const char* pszDefFn)
    {
        ...
        //Properties
        XMLElement* props = root->FirstChildElement("Properties");
        if(props)
        {
            XMLElement* field = props->FirstChildElement();
        ...
        //ClientMethods
        XMLElement* methods = root->FirstChildElement("ClientMethods");
        if(methods)
        {
            XMLElement* m = methods->FirstChildElement();
    ...
    _SEntityDefProperties* CDefParser::ReadProperty(SEntityDef* pDef, XMLElement* node)
    {
        _SEntityDefProperties* pProp = new _SEntityDefProperties;
        pProp->m_name.assign(node->Value());

        XMLElement* field_attri = node->FirstChildElement("Type");
        if(field_attri)
        {
    ...
    _SEntityDefMethods* CDefParser::ReadMethod(XMLElement* node)
    {
        _SEntityDefMethods* p = new _SEntityDefMethods;
        p->m_funcName.assign(node->Value());

        XMLElement* e = node->FirstChildElement("Exposed");
        if(e)
        {
            p->m_bExposed = true;
        }
